...
type FormState = {body: string; occurredAt: string; newTagNames: string};
...
export default function Home() {
    ...
    const [form, setForm] = useState<FormState>({body: '', occurredAt: localDateTime(), newTagNames: ''});
    ...
    const openCreate = () => {
        setEditingId(null);
        setForm({body: '', occurredAt: localDateTime(), newTagNames: ''});
        setIsModalOpen(true);
    };
    ...
    const openEdit = (item: ApiLifeLog) => {
        ...
        setIsModalOpen(true);
        setForm({body: item.body, occurredAt: item.occurredAt.slice(0, 16), newTagNames: item.tags.map((tag) => tag.name).join(', ')});
    };
    ...
    const submit = async (event: FormEvent) => {
        ...
        const response = await fetch(endpoint, {
            ...
            body: JSON.stringify({body: form.body, occurredAt: new Date(form.occurredAt).toISOString(), newTagNames: form.newTagNames.split(',').map((name) => name.trim()).filter(Boolean)}),
        });
