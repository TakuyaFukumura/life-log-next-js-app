...
export default function LocationPicker({location, onChange}: Props) {
    ...
    return (
        <fieldset className="mt-4 rounded border p-3">
            ...
            {location && <p className="mt-2 text-sm text-green-700 dark:text-green-300">位置情報を取得済み（精度: {location.accuracyMeters === null || location.accuracyMeters === undefined ? '不明' : `${Math.round(location.accuracyMeters)}m`}）</p>}
            {error && <p role="alert" className="mt-2 text-sm text-red-600">{error}</p>}
            <div className="mt-2 flex gap-3">
                <button type="button" onClick={getCurrentLocation} disabled={loading} className="rounded border px-3 py-1 text-sm disabled:opacity-50">
                    {loading ? '位置情報を取得しています...' : location ? '現在地を更新' : '現在地を追加'}
                </button>
                {location && <button type="button" onClick={() => onChange(null)} className="rounded border px-3 py-1 text-sm">位置情報を削除</button>}
            </div>
        </fieldset>
    );
}
