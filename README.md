...
- 日時はブラウザのタイムゾーンに依存せず、日本時間（Asia/Tokyo）で入力・表示
...
日時は日本時間（`Asia/Tokyo`）として扱います。画面から登録する場合は、`datetime-local` の入力値を日本時間としてISO 8601形式へ変換して送信します。レスポンスの日時はUTCのISO 8601形式ですが、`timezone` は常に `Asia/Tokyo` です。
